...
- `login_keyboard.py`: PyInstaller용 실행 진입점
...
`login_keyboard.py`는 작은 진입점이지만, PyInstaller가 import 관계를 따라 `login_keyboard_windows/` 패키지를 함께 포함합니다. exe 빌드를 위해 모든 코드를 한 파일에 둘 필요는 없습니다.
